...
MVP form-key usability checkpoint:

```text
DATE: 2026-08-27
SUMMARY: Creating a draft dynamic form now asks for its name, description, and
applicable entity type only. The form's stable key is generated by the server when
the normal UI omits it, so a user does not have to understand or type a persistent
technical identifier. Advanced API clients retain the optional validated key field
for safe integration use.

FILES_CHANGED:
Form-create request schema/service, form designer, API contract/specification,
focused backend/frontend tests, and current status.

DATABASE_CHANGES:
None.

API_CHANGES:
`POST /workspaces/{workspace_id}/forms` now accepts an omitted `key`; the server
returns its generated immutable key in the existing form response.

TESTS_ADDED:
The form-service creation test now proves server generation. The form-designer test
proves the normal creation screen does not render the persistent-key field.

TEST_RESULTS:
Affected backend Ruff formatting/lint/mypy pass; form-service tests pass (11).
Focused form-designer tests pass (3); frontend lint and full TypeScript checks pass.

SECURITY_IMPACT:
Server-side key generation prevents clients from having to devise identifiers while
the server preserves validation and collision rejection for advanced explicit keys.
No workspace, permission, or audit control changes.
