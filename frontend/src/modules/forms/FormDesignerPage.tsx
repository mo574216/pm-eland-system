...
export function FormDesignerPage() {
  ...
  const creation = useForm<CreateValues>({ defaultValues: { name: '', description: '', entityTypeId: '' } })
  const entityTypeId = useWatch({ control: creation.control, name: 'entityTypeId' })
  const create = useMutation({ mutationFn: (values: CreateValues) => createForm(workspaceId ?? '', { name: values.name.trim(), description: values.description.trim() || null, entity_type_id: values.entityTypeId }), onSuccess: async (form) => { creation.reset(); setSelectedId(form.id); await queryClient.invalidateQueries({ queryKey: ['forms', workspaceId] }) } })
