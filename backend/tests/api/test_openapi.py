...
def test_openapi_uses_version_31_and_exposes_health_routes(client: TestClient) -> None:
    ...
    import_upload = document["paths"]["/api/v1/workspaces/{workspace_id}/imports"]["post"]
    assert "multipart/form-data" in import_upload["requestBody"]["content"]
    assert "202" in import_upload["responses"]
